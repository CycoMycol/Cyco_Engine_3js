...
import {
	texture, textureLoad, uv, ivec2, vec2, vec4,
	positionGeometry, reference, varyingProperty, materialReference, Fn
} from 'three/tsl';
...
class LensflareMesh extends Mesh {
	...
	constructor() {
		...
		/**
		 * Overwritten to make sure lensflares are rendered last.
		 * @type {number}
		 * @default Infinity
		 */
		...
		const vertexNode = vec4(
			positionGeometry.xy.mul( scale ).add( screenPosition.xy ),
			screenPosition.z,
			1.0
		);
		...
		material2.vertexNode = Fn( ( { material } ) => {
			...
			return vec4(
				pos.mul( scale ).add( screenPosition.xy ).xy,
				screenPosition.z,
				1.0
			);
